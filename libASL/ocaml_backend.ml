...
let name ~use_pc = if use_pc then "offlineASL_pc" else "offlineASL"
let fullname ~use_pc file = name ~use_pc ^ "_" ^ file


(* Write an instruction file, containing just the behaviour of one instructions *)
let write_instr_file ~use_pc fn fnsig dir =
  let m = fullname ~use_pc (name_of_FIdent fn) in
...
let write_test_file ~use_pc tests dir =
  let m = fullname ~use_pc "decode_tests" in
  let path = dir ^ "/" ^ m ^".ml" in
  let oc = open_out path in
  let st = init_st oc in
  write_preamble global_dep_opens st;
...
let write_new_dune_file ~use_pc decoder files global_deps dir  : unit =
  ...
  let global_deps_str = String.concat " " global_deps in

  let name = name ~use_pc in
  let public_name = if use_pc then "pc_aarch64" else "aarch64" in
  let compile_flags = "(flags (:standard -w -27 -w -33))" in
  Printf.fprintf oc "
    (library
      (name %s)
      ...
      (modules %s)
      (libraries %s \n"
    name public_name compile_flags decoder global_deps_str;
  ...
  let write_library file ~deps =
    Printf.fprintf oc "
      (library
        ...
        (public_name aslp_offline.%s)
        (libraries %s))\n\n"
      file file compile_flags file deps in

  List.iter (write_library ~deps:global_deps_str) files;
...
let run config dfn dfnsig tests fns =
  ...
  let use_pc = config.use_pc in
  let files = Bindings.fold (fun fn fnsig acc -> (write_instr_file ~use_pc fn fnsig dir)::acc) fns [] in
  let files = (write_test_file ~use_pc tests dir)::files in
  let decoder = write_decoder_file config.use_pc dfn dfnsig files dir in
  try
  write_new_dune_file ~use_pc decoder files global_deps dir
  with
